...
public class JavaCord {
    ...
    private static final String DISCORD_BOT_TOKEN_ENV = "PSYCHO_DISCORD_BOT_TOKEN";
    private static final String LEGACY_DISCORD_BOT_TOKEN_ENV = "KANDARIN_DISCORD_BOT_TOKEN";

    private static String serverName = GameSettings.RSPS_NAME;

    private static DiscordApi api = null;

    public static CompletableFuture<Void> init() {
        String token = environment(DISCORD_BOT_TOKEN_ENV, LEGACY_DISCORD_BOT_TOKEN_ENV);
        if (token == null || token.trim().isEmpty()) {
            log.warning("Discord bot is enabled, but PSYCHO_DISCORD_BOT_TOKEN is not set. Skipping Discord bot login.");
