		...
		if(energyType != null) {
			...
			player.setCurrentTask(new Task(cycle, player, false) {
				...
				public void execute() {
					...
					n.dealDamage(new Hit(null, Misc.getRandom(12), Hitmask.RED, CombatIcon.MAGIC));
					if(Misc.getRandom((30 + gearboost(player))) <= 10) {
						player.dealDamage(new Hit(null, 1 + Misc.getRandom(48), Hitmask.RED, CombatIcon.DEFLECT));
