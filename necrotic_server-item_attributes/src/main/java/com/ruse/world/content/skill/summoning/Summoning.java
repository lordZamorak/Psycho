...
public class Summoning {
	...
	public void summonPet(BossPets.BossPet bossPet, boolean login) {
		...
		for (int i = 0; i < BossPets.BossPet.values().length; i++) {
			if (BossPets.BossPet.values()[i].spawnNpcId == bossPet.spawnNpcId) {
				...
				if (!player.getBossPet(i)) {
					player.getPacketSender().sendMessage("You've unlocked "+ItemDefinition.forId(BossPets.BossPet.values()[i].itemId).getName()+", it can be re-purchased from the Zoo Keeper if lost.");
	...
	public void unsummon(boolean full, boolean dropItems) {
		...
		if(getFamiliar() != null && getFamiliar().getSummonNpc() != null) {
			...
			if(full) {
				if(BossPets.BossPet.forSpawnId(spawnId) != null) {
					player.getInventory().add(BossPets.BossPet.forSpawnId(spawnId).itemId, 1);
				}
			}
		}
		if(full) {
