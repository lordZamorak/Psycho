...
public class Construction {
	...
	public static void newHouse(Player p) {
		...
		HouseFurniture pf = new HouseFurniture(7, 7, 0, ConstructionData.HotSpots.CENTREPIECE.getHotSpotId(), ConstructionData.Furniture.EXIT_PORTAL.getFurnitureId(), ConstructionData.HotSpots.CENTREPIECE.getXOffset(), ConstructionData.HotSpots.CENTREPIECE.getYOffset());
			...
			for (int x = 0; x < 13; x++) {
				for (int y = 0; y < 13; y++) {
					...
					if (p.getHouseRooms()[z][x][y].getX() == 0)
						continue;
					Palette.PaletteTile tile = new Palette.PaletteTile(p.getHouseRooms()[z][x][y].getX(), p.getHouseRooms()[z][x][y].getY(), p.getHouseRooms()[z][x][y].getZ(), p.getHouseRooms()[z][x][y].getRotation());
	...
	public static void createDungeonPalette(Player p) {
		...
		for (int x = 0; x < 13; x++) {
			for (int y = 0; y < 13; y++) {
				Palette.PaletteTile tile = null;
				if (p.getHouseRooms()[4][x][y] == null) {
					tile = new Palette.PaletteTile(ConstructionData.RoomData.DUNGEON_EMPTY.getX(), ConstructionData.RoomData.DUNGEON_EMPTY.getY(), 0, 0);
				} else {
					tile = new Palette.PaletteTile(p.getHouseRooms()[4][x][y].getX(), p.getHouseRooms()[4][x][y].getY(), p.getHouseRooms()[4][x][y].getZ(), p.getHouseRooms()[4][x][y].getRotation());
				}
		...
		for (int z = 0; z < 4; z++) {
			for (int x = 0; x < 13; x++) {
				for (int y = 0; y < 13; y++) {
					...
					if (p.getHouseRooms()[z][x][y].getX() == 0)
						continue;
					Palette.PaletteTile tile = new Palette.PaletteTile(
	...
	public static void placeNPCs(Player p) {
		...
		for (HouseFurniture pf : p.getHouseFurniture()) {
			ConstructionData.Furniture f = ConstructionData.Furniture.forFurnitureId(pf.getFurnitureId());
			...
			ConstructionData.HotSpots hs = ConstructionData.HotSpots.forHotSpotIdAndCoords(pf.getHotSpotId(),
	...
	public static void doFurniturePlace(ConstructionData.HotSpots s, ConstructionData.Furniture f,
                                        ArrayList<ConstructionData.HotSpots> hsses, int[] myTiles, int actualX, int actualY,
			...
			if (s.getXOffset() == 0) {
				for (Portal portal : p.getRegionInstance().getOwner().getHousePortals()) {
					if (portal.getRoomX() == myTiles[0] - 1 &&
						...
						if (ConstructionData.Portals.forType(portal.getType()).getObjects() != null)
							portalId = ConstructionData.Portals.forType(portal.getType()).getObjects()[f.getFurnitureId() - 13636];
					...
					if (isEdge)
						p.getPacketSender().sendObject_cons(
								...
								placeBack ? s.getObjectId() + 2 : f
										.getFurnitureId(),
								ConstructionData.HotSpots.getRotation_2(rot, roomRot), 22,
								height);
					else if (isWall)
						p.getPacketSender().sendObject_cons(
								offsetX,
								...
								placeBack ? s.getObjectId() + 1 : f
										.getFurnitureId() + 1,
								ConstructionData.HotSpots.getRotation_2(rot, roomRot),
	...
	public static String hasReqs(Player p, ConstructionData.Furniture f, ConstructionData.HotSpots hs) {
		...
		if (f.getFurnitureRequired() != -1) {
			ConstructionData.Furniture fur = ConstructionData.Furniture.forFurnitureId(f.getFurnitureRequired());
			int[] myTiles = getMyChunk(p);
			for (HouseFurniture pf : p.getHouseFurniture()) {
	...
	public static boolean buildActions(Player p, ConstructionData.Furniture f, ConstructionData.HotSpots hs) {
		...
		switch (buttonId) {
			...
			case 2494:
				if (p.getDialogueActionId() == 646) {
					String s = ConstructionData.Portals.VARROCK.canBuild(p);
					if (s != null) {
						p.getPacketSender().sendInterfaceRemoval();
				...
				if (p.getDialogueActionId() == 647) {
					String s = ConstructionData.Portals.ARDOUGNE.canBuild(p);
					if (s != null) {
						p.getPacketSender().sendInterfaceRemoval();
						DialogueManager.start(p, ConstructionDialogues.sendConstructionStatement(s));
					}
			...
			case 2495:
				...
				if (p.getDialogueActionId() == 646) {
					String s = ConstructionData.Portals.LUMBRIDGE.canBuild(p);
					if (s != null) {
						p.getPacketSender().sendInterfaceRemoval();
						DialogueManager.start(p, ConstructionDialogues.sendConstructionStatement(s));
					}
				...
				if (p.getDialogueActionId() == 647) {
					String s = ConstructionData.Portals.YANILLE.canBuild(p);
					if (s != null) {
						p.getPacketSender().sendInterfaceRemoval();
						DialogueManager.start(p, ConstructionDialogues.sendConstructionStatement(s));
					}
			...
			case 2496:
				if (p.getDialogueActionId() == 646) {
					String s = ConstructionData.Portals.FALADOR.canBuild(p);
					if (s != null) {
						p.getPacketSender().sendInterfaceRemoval();
						DialogueManager.start(p, ConstructionDialogues.sendConstructionStatement(s));
					}
				...
				if (p.getDialogueActionId() == 647) {
					String s = ConstructionData.Portals.KHARYLL.canBuild(p);
					if (s != null) {
						p.getPacketSender().sendInterfaceRemoval();
						DialogueManager.start(p, ConstructionDialogues.sendConstructionStatement(s));
					}
			...
			case 2497:
				if (p.getDialogueActionId() == 646) {
					String s = ConstructionData.Portals.CAMELOT.canBuild(p);
					if (s != null) {
						p.getPacketSender().sendInterfaceRemoval();
						DialogueManager.start(p, ConstructionDialogues.sendConstructionStatement(s));
					}
				...
				if (p.getDialogueActionId() == 647) {
					String s = ConstructionData.Portals.EMPTY.canBuild(p);
					if (s != null) {
						p.getPacketSender().sendInterfaceRemoval();
						DialogueManager.start(p, ConstructionDialogues.sendConstructionStatement(s));
					}
			...
			case 2461:
				if (p.getDialogueActionId() == 654) {
					...
					p.getHouseRooms()[0][7][7] = new Room(0, ConstructionData.GARDEN, 0);
					HouseFurniture pf = new HouseFurniture(7, 7, 0, ConstructionData.HotSpots.CENTREPIECE.getHotSpotId(),
							ConstructionData.Furniture.EXIT_PORTAL.getFurnitureId(), ConstructionData.HotSpots.CENTREPIECE.getXOffset(),
							ConstructionData.HotSpots.CENTREPIECE.getYOffset());
					p.getHouseFurniture().add(pf);
					DialogueManager.start(p, 354);
					return true;
				}
				if (p.getDialogueActionId() == 457) {
					ConstructionData.Butlers b = ConstructionData.Butlers.forId(p.getHouseServant());
				...
				if (p.getDialogueActionId() == 444) {
					ConstructionData.Butlers b = ConstructionData.Butlers.forId(((NPC) p.getInteractingEntity()).getId());
	...
	public static boolean handleRemoveClick(int obX, int obY, int objectId,
		...
		ConstructionData.Furniture f = ConstructionData.Furniture.forFurnitureId(objectId);
		if (f == null)
			return false;
		if (f == ConstructionData.Furniture.EXIT_PORTAL || f == ConstructionData.Furniture.EXIT_PORTAL_) {
			int portalAmt = 0;
			for (HouseFurniture pf : p.getHouseFurniture()) {
				ConstructionData.Furniture ff = ConstructionData.Furniture.forFurnitureId(pf.getFurnitureId());
				if (ff == ConstructionData.Furniture.EXIT_PORTAL || ff == ConstructionData.Furniture.EXIT_PORTAL_)
					portalAmt++;
			}
			if (portalAmt < 2) {
		...
		if (objectId == 13331) {
			hs = ConstructionData.HotSpots.OUBLIETTE_FLOOR_1;
		}
		if (objectId == 13313) {
			hs = ConstructionData.HotSpots.OUBLIETTE_CAGE_1;
		}
		if (objectId == 13126 || objectId == 13127 || objectId == 13128
			...
			hs = ConstructionData.HotSpots.COMBAT_RING_1;
		}
		if (objectId == 13456)
			if (room.getType() == ConstructionData.FORMAL_GARDEN)
				hs = ConstructionData.HotSpots.FORMAL_HEDGE_1;
		if (objectId == 13449)
			if (room.getType() == ConstructionData.FORMAL_GARDEN)
				hs = ConstructionData.HotSpots.FORMAL_FENCE;
		if (objectId == 15270 || objectId == 15273 || objectId == 15274
				|| objectId >= 13588 && objectId <= 13597) {
			if (room.getType() == ConstructionData.CHAPEL)
				hs = ConstructionData.HotSpots.CHAPEL_RUG_1;
			if (room.getType() == ConstructionData.PARLOUR)
				hs = ConstructionData.HotSpots.PARLOUR_RUG_3;
			if (room.getType() == ConstructionData.SKILL_ROOM
					...
					|| room.getType() == ConstructionData.SKILL_HALL_DOWN)
				hs = ConstructionData.HotSpots.SKILL_HALL_RUG_3;
	...
	public static boolean handleSpaceClick(int obX, int obY, int objectId,
		...
		if (hs == null)
			return true;
		ArrayList<ConstructionData.Furniture> f = ConstructionData.Furniture.getForHotSpotId(hs
				.getHotSpotId());
		if (f == null)
			return false;
	...
	public static void createRoom(int roomType, Player p, int toHeight) {
		ConstructionData.RoomData rd = ConstructionData.RoomData.forID(roomType);
		if (rd == null)
			return;
		...
		if (toHeight == 100) {
			...
			doFurniturePlace(ConstructionData.HotSpots.SKILL_HALL_STAIRS_1,
					ConstructionData.Furniture.forFurnitureId(stairId), null, myTiles,
		...
		if (toHeight == 101) {
			...
			doFurniturePlace(ConstructionData.HotSpots.SKILL_HALL_STAIRS,
					ConstructionData.Furniture.forFurnitureId(stairId), null, myTiles,
					ConstructionData.BASE_X + (myTiles[0] * 8) + 3,
					ConstructionData.BASE_Y + (myTiles[1] * 8) + 3, rotation,
					p, false, 1);
		...
		Palette.PaletteTile tile = new Palette.PaletteTile(room.getX(), room.getY(),
...
*/
	...
	public static void handleInterfaceItems(ArrayList<ConstructionData.Furniture> items, Player c) {
		c.getPacketSender().sendConstructionInterfaceItems(items);
	}

	public static void handleInterfaceCrosses(ArrayList<ConstructionData.Furniture> items,
											  Player c, ConstructionData.HotSpots hs) {
		int i = 1000;

		for (ConstructionData.Furniture f : items) {
			...
			if (f.getFurnitureRequired() != -1) {
				ConstructionData.Furniture fur = ConstructionData.Furniture.forFurnitureId(f
	...
	public static void handleItemClick(int itemID, Player p) {
		...
		if (!p.isBuildingMode())
			return;
		ConstructionData.Furniture f = ConstructionData.Furniture.forItemId(itemID);
		if (f == null)
			return;
