...
public class CacheDownloader {
	...
	private static final String[] REQUIRED_CACHE_FILES = {
			"main_file_cache.dat",
	...
	public static boolean updatedCache() {
		if (Configuration.developerMode) {
			...
			try {
				if (hasRequiredCacheFiles() && !forceUpdateCache()) {
					UPDATING = false;
					return true;
				}
				if (Configuration.STOP_CACHE_UPDATES) {
					System.err.println("Cache update is disabled and required cache files are missing from " + Signlink.getCacheDirectory());
					UPDATING = false;
					return false;
				}
	...
	private static boolean hasRequiredCacheFiles() {
		File cacheDirectory = new File(Signlink.getCacheDirectory());
		for (String requiredCacheFile : REQUIRED_CACHE_FILES) {
			File file = new File(cacheDirectory, requiredCacheFile);
